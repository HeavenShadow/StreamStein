...
export const getSourceUrl = (
  ...
  const src =
    PLAYER_SOURCES.find((s) => s.id === normalizedSourceId) ?? PLAYER_SOURCES[0];
  const baseUrl =
...
export const sourceSupportsProgress = (sourceId) =>
  PLAYER_SOURCES.find((s) => s.id === normalizeSourceId(sourceId))
    ?.supportsProgress ?? false;

export const sourceProgressViaFrames = (sourceId) =>
  PLAYER_SOURCES.find((s) => s.id === normalizeSourceId(sourceId))
    ?.progressViaFrames ?? false;

export const sourceIsAsync = (sourceId) =>
  PLAYER_SOURCES.find((s) => s.id === normalizeSourceId(sourceId))?.async ??
  false;

export const getNextNonAsyncSource = (currentId) => {
  const normalizedCurrentId = normalizeSourceId(currentId);
  const nonAsync = PLAYER_SOURCES.filter((s) => !s.async);
  if (!nonAsync.length) return null;
  const idx = nonAsync.findIndex((s) => s.id === normalizedCurrentId);
